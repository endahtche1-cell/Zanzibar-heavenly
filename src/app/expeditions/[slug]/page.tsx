...
export async function generateStaticParams() {
  return getAllExpeditions().map((e) => ({ slug: e.slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params
  const exp = getExpeditionBySlug(slug)
  if (!exp) return {}
  return {
    title: exp.seo.title,
    description: exp.seo.description,
    openGraph: { title: exp.seo.title, description: exp.seo.description, images: [{ url: exp.heroImage }] },
  }
}
...
export default async function ExpeditionDetailPage({ params }: Props) {
  ...
  const whatsappUrl = getWhatsAppEnquiryUrl(exp.title, 'expedition')
  ...
  return (
    <>
      ...
      <section className="relative min-h-[50vh] overflow-hidden">
        <ImagePlaceholder src={exp.heroImage} alt={exp.title} fill priority className="object-cover" />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-12">
          <div className="mx-auto max-w-7xl">
            ...
            <h1 className="mt-3 font-heading text-3xl font-normal text-white sm:text-4xl lg:text-5xl">{exp.title}</h1>
            <div className="mt-3 flex flex-wrap items-center gap-4 text-sm text-white/60">
              <span>{exp.location}</span>
              <span>&middot;</span>
              <span>{exp.duration}</span>
              <span>&middot;</span>
              <span>From {formatPrice(exp.priceFrom, exp.currency)}</span>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-12 lg:grid-cols-3">
            ...
            <div className="lg:col-span-2 space-y-12">
              ...
              <FadeIn>
                <div>
                  <p className="text-lg font-light leading-relaxed text-muted">{exp.content}</p>
                </div>
              </FadeIn>
              ...
              {exp.itinerary.length > 0 && (
                <FadeIn delay={0.2}>
                  <div>
                    <p className="text-[11px] tracking-[0.2em] uppercase text-muted">Journey</p>
                    <h2 className="mt-2 font-heading text-3xl font-normal text-ink">Day-by-Day Itinerary</h2>
                  ...
                  <div>
                    ...
                    <ul className="mt-4 flex flex-wrap gap-2">
                      {exp.whatToBring.map((item) => (
                        <li key={item} className="border border-border bg-warmwhite px-4 py-1.5 text-sm text-muted">{item}</li>
                      ))}
                    </ul>
                  </div>
                </FadeIn>
                ...
                <FadeIn direction="right" delay={0.1}>
                  <div className="border border-border bg-sand/50 p-6">
                    <div className="text-center">
                      ...
                      <p className="mt-1 text-3xl font-light text-teal">{formatPrice(exp.priceFrom, exp.currency)}</p>
                    ...
                    <div className="mt-6 space-y-3">
                      <a
                        ...
                        className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-teal px-6 py-3 text-[13px] font-light tracking-[0.12em] uppercase text-white transition-all hover:bg-teal-dark hover:shadow-lg"
                      >
                      ...
                      <Link
                        href="#enquiry-form"
                        className="flex w-full items-center justify-center rounded-full bg-teal px-6 py-3 text-[13px] font-light tracking-[0.12em] uppercase text-white transition-all hover:bg-teal-dark hover:shadow-lg"
                      >
                        Send Enquiry
                      </Link>
                    </div>
                  ...
                  <div className="border border-border bg-sand/50 p-6">
                    ...
                    <dl className="mt-4 space-y-3 text-sm">
                      <div className="flex justify-between">
                        ...
                        <dd className="font-medium text-ink">{exp.duration}</dd>
                      </div>
                      <div className="flex justify-between">
                        <dt className="text-muted">Difficulty</dt>
                        <dd className="font-medium text-ink">{exp.difficulty}</dd>
                      </div>
                      <div className="flex justify-between">
                        <dt className="text-muted">Group Size</dt>
                        <dd className="font-medium text-ink">{exp.groupSize}</dd>
                      </div>
                      <div className="flex justify-between">
                        <dt className="text-muted">Best Time</dt>
                        <dd className="font-medium text-ink">{exp.bestTimeToGo}</dd>
                      </div>
                    </dl>
                    {exp.pickupInfo && <p className="mt-4 border-t border-border pt-4 text-xs text-muted">{exp.pickupInfo}</p>}
                    <p className="mt-2 text-xs text-muted">{exp.cancellationPolicyShort}</p>
                  </div>
                </FadeIn>
              </div>
            </aside>
          ...
          <FadeIn delay={0.1}>
            <div className="mt-20 border border-border bg-sand/50 p-8" id="enquiry-section">
              ...
              <h2 className="mt-2 font-heading text-3xl font-normal text-ink">Enquire About This Expedition</h2>
